...
class OnionProcessor(ColaProcessor):
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):
            ...
            # np.random.shuffle(tmp_right)
            # right = " ".join(tmp_right)            
            if np.random.rand() > 0.5:
                left, right = right, left
                target = 1
            
            # text_a = tokenization.convert_to_unicode(left)
            # text_b = tokenization.convert_to_unicode(right)
            text_a = left
            text_b = right
            label = str(target)
            examples.append(
